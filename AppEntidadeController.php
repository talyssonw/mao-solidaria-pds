			...
			if($this->input->post()!= null)
			{
				...
				if(!$this->upload->do_upload($img)){   
					rmdir('assets/files/uploads/'.preg_replace("/[^0-9]/","", $this->input->post('entCnpj')));
      				$this->session->set_flashdata('errorCad', '<div class="alert alert-danger">Imagem excedeu a resolução máxima (500x500) ou tamanho máximo (2MB).</div>');
      				$this->template->load('template', 'cadEntidadeView');  
				...
				else
				{
					...
					$url = 'assets/files/uploads/'.preg_replace("/[^0-9]/","", $this->input->post('entCnpj')).'/'.$url;
					$entApp = [
					'cnpj' => preg_replace("/[^0-9]/","", $this->input->post('entCnpj')),
					'name' => $this->input->post('entName'),
					'email'=> $this->input->post ('entEmail'),
	...
	public function change_profile_picture_event()
	{
		...
		$config['file_name'] = $url;
				...
				else
				{
					...
					$url = 'assets/files/uploads/'.$this->session->userdata('cnpj').'/'.$url;
					$image_update = [
					'urlImg' => $url];
